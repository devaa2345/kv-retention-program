...
ph = json.load(open('results/posthoc.json'))
err = {int(k): v for k, v in ph['intercepted_monotonicity']['error_pct_by_bits'].items()}
...
for w in [8, 7, 6, 5, 4, 3]:
    ...
    print(f"{w:>4} {len(rs):>4} {statistics.mean(acc):>9.4f} {empty:>10.3f} {rep:>11.3f} "
          f"{degen:>6} {clean:>6} {mixed:>6} {err.get(w,float('nan')):>9.2f}% "
          f"{str(e.get('max_levels'))+'/'+str(e.get('limit')):>12} {e.get('violations'):>5}")
...
json.dump(table, open('results/sweep_table.json', 'w'), indent=2)
print("\nwrote results/sweep_table.json")
