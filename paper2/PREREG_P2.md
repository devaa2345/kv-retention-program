...
>
> All nine decisions are resolved; B8 and B9 are resolved; the budget ladder is extended to six
> cells; M1 is excluded. The sha256 in §12 is the committed baseline.
>
> **Neither session edits this document again.** If either believes it needs changing, it
> **stops and writes to `STATUS.md`** rather than editing. Paper 1's rule applies: a run against
> a modified instrument is a **new pre-registration**, not an amendment to this one.
>
> The one item deliberately left open is the **finding-1 slot in §11, marked NOT APPLIED —
> UNRESOLVED**. Nothing downstream depends on it.
>
> Stage 3 (N1, the random-span control) is unblocked by this freeze and is a hard gate.
...
| `oracle_prescient` | queried fact's gold spans + filler from `floor_pos` | absolute bound; gives `I` |
...
Computed over this file with §12's `sha256:` line held at the placeholder `<pending>`, so the
value is reproducible: restore that line to `<pending>` and re-hash to verify.

```
sha256: 8ac3895709be0721656cef0c705cf843302245ab6d841373ee1676032db203e9
frozen: 2026-09-06
method: LF-normalised UTF-8 bytes of this file, with the sha256 line held at "<pending>"
```
